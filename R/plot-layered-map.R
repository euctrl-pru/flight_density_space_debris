...
gtfs <- gtfs2gps::read_gtfs(system.file(
  "extdata/poa/poa_eptc.zip",
  package = "r5r"
))
...
temp2 <- temp1 +
  ...
  # schools
  geom_sf(
    data = hex %>% rotate_data(y_add = .2),
    color = 'gray50',
    fill = NA,
    size = .1
  ) +
  geom_sf(
    data = schools %>% rotate_data(y_add = .2),
    color = '#0f3c53',
    size = .1,
    alpha = .8
  ) +
  ...
  # hospitals
  geom_sf(
    data = hex %>% rotate_data(y_add = .3),
    color = 'gray50',
    fill = NA,
    size = .1
  ) +
  geom_sf(
    data = hospitals %>% rotate_data(y_add = .3),
    color = '#d5303e',
    size = .1,
    alpha = .5
  ) +
  ...
  # OSM
  geom_sf(
    data = roads2 %>% rotate_data(y_add = .4),
    color = '#019a98',
    size = .2
  ) +
  ...
  new_scale_color() +
  geom_sf(
    data = subset(landuse, P001 > 0) %>% rotate_data(y_add = .6),
    aes(fill = CMATT30),
    color = NA,
    show.legend = FALSE
  ) +
