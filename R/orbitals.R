...
kepler_to_cartesian <- function(
  ...
  p <- a * (1 - e^2) # semiparameter (semilatus rectum)
  r_0 = p / (1 + e * cos(nu))
...
# rE <- 6378e3 # Earth equatorial radius
# alt <- 550e3 # altitude of satellite, i.e. 500 km for LEO
# ecc <- 0 # circlar orbit when uncontrolled reentry of debris
# mSat <- 0 # mass of debris/satellite << mass Earth, so it can be 0
# Omega <- 0
# w <- 0
# i <- 10
#
