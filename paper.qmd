...
evaluation of the collision and casualty risk for which air traffic density is
a key component.
...
At the same time, the trajectory of a falling re-entering debris object at regular en-route flight levels will be a vertical decent movement[^ballistic_descent].
...
The values $W_t$, $H_t$, $L_t$ and $C_{t}$ have been retrieved from [@performance_database;@skybrary;@doc8643]; we used $\varepsilon_t = \qty{1000}{m^2}$ missing entries or unknown aircraft types.
...
This denotes the intersection point of the object crossing the plane of Earth's equator if moving from the southern hemnisphere to the nothern one.
