...
format:
  ieee-pdf:
    ...
    include-before-body:
      - text: |
          \let\qty\SI
          % NOTE: make siunitx's \qty work without clashing with 
          %      the one in physics
  ieee-html: 
...
work. 
<!-- results / conclusions --> This work enhances the previous model by using
...
with population density [@patera2008] or aircraft transponder data[^adsb]
[@wrightAirspaceClosuresDue2025], we use actual flown flight
trajectories.
This provides a more realistic figure of the number of affected air traffic.
Thus, flight density is directly related to the number of flight operations per 
volume of airspace.

[^adsb]: Crowdsourced ADS-B/Mode S data suffer from lack of coverage away from landmasses and from the need to filter away position reports for stationary aircraft at airports which unnecessarily inflate the density at the location.

This work focusses on the European region. The hourly flight density
estimation is based on _flown_ \acr{IFR} trajectories as recorded
by EUROCONTROL's \acr{NM}[^1]. These trajectories are reconstructed from
the filed \acr{FP} and recalculated when big deviations are detected from
position reports received from \acr{ATC}.

To decrease spatial complexity, the data is
...
The choice of hex resolution is a compromise between having to use
highly detailed trajectories (more 4D positions and more processing and
storage) and capturing long enough portions of flight inside the cell.


To build an upper threshold for the flight impact exposure, this study uses the
busiest week during summer 2023 and 2024.
This ensures a reasonable value, as peak airspace utilisation provides
a handle on the air traffic operations and interactions of the different
aircraft as part of wider *'traffic flows'*.
The flight densities are characterised on an hourly basis for all days
in these weeks per H3-cell.
...
Flight density in the EUROCONTROL Area on the 5th of July 2024 (the busiest day of the busiest summer week in 2024) at H3 cells resolution $3$. The black hexagon in the South of UK marks the densest cell at 24.56 flights/hour, just around the airport of London Heathrow.
:::


@fig-density-summer2024-res-3 shows an example of binning the hourly flight
density on the 5th of July 2024 using H3 hexagons.
The chart depicts the expected outcomes in terms of major
...
Uber's H3 method ensures non-overlapping hexagons (and 12 pentagons), with an equidistant relationship between all neighbours, and enables an efficient indexing of the cells. 
...
where $s_{a}$ is the average aircraft speed, 
...
* constant debris fall speed $s_{d_k} = \qty{145}{mph} = S$ 
  ([we adopt this value from pages 7-8 in @patera2008])
...
(and $pax = 7$)[^pax] for missing entries which usually are due to smaller aircraft not listed in the above sources.


[^pax]: `pax` values, i.e. number of passengers, will be used for future work on
...
Impact probability density $\sigma(\varphi, i = 51.95)$ for debris of
...
This denotes the intersection point of the object crossing the plane of Earth's equator if moving from the southern hemisphere to the northern one.
...
$\ang{51.95}$ is plotted in @fig-weighting-inclination-i and takes a characteristic U shape with zero values for latitudes beyond $i$.
...
$$ {#eq-debris-impact-probability-historical}
...
![Hourly weightings for H3 cells, $W_H(h)$, in \acr{NM} area.](media/figures/weighting_on_Earth.png){#fig-weighting-nm-area out-width=100%}
...
## Traffic density per cell

In order to derive $\rho_t(h)$, we have taken the trajectories of flown flights
in the \acr{NM} Area, resampled at $\qty{30}{s}$ so that the calculation of the
occupancy, i.e. the time spent in the hex, per aircraft type is just a matter
of counting the number of points per cell.
This binning operation is extremely fast to perform in Uber's H3 hierarchical
geospatial indexing system and a real advantage in processing the huge amount of
data point representing the flight trajectories.




For July, $5^{th}$ 2024 alone, we extracted $37721$ flights with
$33,482,686$ 4D positions (at $\qty{30}{s}$ sampling), where 19 aircraft types
cover each at least 1% of the total and the remaining 266 (!) types account
for the residual 20.8%, see @fig-aircraft-types.



## Hourly impact probability per H3 cell

When combining the hourly ($H$) aircraft occupancy (per aircraft type) with the
relevant weightings per $h$ cell, $W_H(h)$, and the effective exposed
area, $\varepsilon_t$, we can numerically compute the
hourly impact probability for each hexagonal cell as:

$$
 E_{h, H} =  \sum_t W_H(h) \cdot \rho_{t,H}(h) \cdot \varepsilon_t
$$ {#eq-impact-probability-hourly}

where $E_{h,H}$ is the collision expectation in the hexagon $h$ at hour $H$.


::: {#fig-aircraft-types}
```{r aircraft-types-july-2024}
#| out-width: "100%"
#| echo: false
knitr::include_graphics("media/figures/aircraft_types.png")
```

Aircraft types on 2024-07-05 in \acr{NM} Area. 
:::
...
knitr::include_graphics(here::here(
  "media",
  "figures",
  "collision_expectation_2024-07-05_3.png"
))
```
...
While this paper was being written, the KOSMOS 482 Descent Craft was being
monitored for an uncontrolled re-entry, forecast at 06:39 May 10, 2025
($\pm 1.5$ hours) [@kosmos482reentry].

![Venera-4 lander mock-up at the Memorial Museum of Cosmonautics. The Kosmos 482 Descent Craft is probably similar.](media/venera-capsule-in-museum.jpg){#fig-kosmos width=80%}

This is the lander module from a 1972 failed USSR Venera mission
[@SpaceReviewKosmos], which is reported to weigh about $\qty{480}{kg}$ and
to have a diameter of approximately $\qty{1}{m}$.
Being built to survive Venus's atmosphere which is much denser and hotter than
Earth's one (surface pressure 93 times that of Earth and temperatures reaching
\ang{467}C (\ang{872}F)), it could re-enter Earth intact and not break into
smaller fragments some of which could burn up.

Knowing that the debris inclination is $\ang{51.95}$, 
see @fig-weighting-inclination-i, we can derive the maximum
collision expectation using data for the busiest day in the busiest 2024 week
as a proxy for the traffic density on the day of re-entry.
Using the approach in this study and the weightings for this specific debris,
the cell with the highest impact risk is cell, `83195dfffffffff` 
(located North-West of London and overlapping with the airport of London Heathrow), with a daily value of probability collision risk of $2.63 \cdot  10^{-10}$.
This corresponds to an annual probability of a collision between this rocket
body and an aircraft of $9.6 \cdot  10^{-8}$ (1 in 10420000).
The risk is very low, for a comparison the odds of a person being struck by
lightning in a given year are less than one in a million
[@cdcLightningStrikeVictim2024].

Radar systems over Germany saw the descent craft at approximately 04:30 UTC and
06:04 UTC on May $10^{th}$. Given that the descent craft was not spotted at
the next expected pass at 07:32 UTC, it is likely that the re-entry already occurred before.

In general case when the re-entry track for a debris would be known
with some lead time, the H3 cells around, say,  \qty{35}{km} on each side
could be identified to retrieve the values of collision expectancy.
It would then be possible to decide whether it would advisable to close
the airspace and re-route traffic around the affected area.
...
The processing of data for 1 day takes less than 30 minutes from
...
Space objects and debris re-entry is an increasing risk to aviation and the
general public.
The higher pace of commercial space operations will see an increase of launches 
and subsequent re-entries.
Dependent on the mission, objects re-entering the atmosphere can take on
different forms, shapes, and sizes.
On top, a higher number of these entries may be uncontrolled.
With the continual growth of air traffic, there is a need to enhance aviation
safety risk awareness and respond in a more tailored fashion to accommodate
the associated uncertainties, reduce the impact on aviation, and limit the risk
to the travelling public.

<!-- what was shown in the paper ------------------------------------------------------------>

This paper expands on previous work. With a focus on Europe, we establish an
upper bound for the traffic density based on actual flights.
...
expectation is determined using the busiest day in 2024 as a representative
proxy of the traffic on the day of re-entry.
...
This cell indexing approach reduces the computation burden significantly.

<!-- future work ---------------------------------------------------------------------------->

Future work will address the derivation of casualty expectation using
...
to explore the impact of megaconstellations and flight growth forecast and
what would happen in the mid- to long-term in various scenarios of
...
::: {#fig-orbital-elements layout="[40,-4,56]" fig-env="figure*" fig-pos="h" height=80%}
...
Orbital elements, Figures 4.1.4-2 and 4.1.4-9 from [@sellers2003understanding].
:::
...
# Appendix: Orbital Elements {#sec-orbitals .appendix}
...
The orbital elements describing the trajectory of an orbiting object are
illustrated in association with an inertial reference frame
$\hat{I}$-$\hat{J}$-$\hat{K}$. 
The $I$-axis, $\hat{I}$, points toward the vernal equinox and 
the $J$-axis, $\hat{J}$, is perpendicular to the $I$-axis in the equatorial plane
in the sense of Earth's rotation, with the $K$-axis pointing up.
The semi-major axis, $a$, the eccentricity, $e$, the inclination, $i$,
the right ascension of the ascending node, $\Omega$, and the argument of perigee, $\omega$, constitute, together with the mean anomaly at the initial position,
$M_0$, a set of six constants of motion that are preserved in the
absence of perturbation. The true anomaly, $\nu$, specifies the location of
...
orbiting object's motion.


The angular position, $\theta$, of the re-entry debris taken from the ascending
...
In the derivation of the debris' orbital positions, we use the value of the Earth's mass, $M_{\oplus} = \qty{5.9722e24}{kg}$ but neglect that of the debris
because it is many orders of magnitudes smaller (and not always known).

Of all the orbital elements, for the needs of this paper to numerically compute
the weighting function, , @fig-weighting-inclination-i, we only need
the inclination $i$. 
For debris with inclination $i$ we simulate an orbit of $N$, i.e. $10000$,
positions ($\nu$) and calculate the corresponding Cartesian positions and
the equivalent longitudes and latitudes.
The latitudes are then binned on half degree intervals to compute
$\sigma(\varphi, i)$, as per (@eq-impact-probability-density).


[^ascending-node]: For an earth satellite, the ascending node is the point in its orbit where the satellite crosses the equatorial plane going from south to north.
