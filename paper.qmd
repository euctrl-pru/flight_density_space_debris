...
This is showcased by an increasing number of activities aimed to generate value through the exploration of space ranging from scientific understanding and experimentation, sustainable production of goods in a non-gravity environment, services, and recently leisure.
...
In general, the method for estimating the risk to commercial aircraft from re-entering debris objects revolves around estimating the exposure of a single flight and the population of flights to the re-entering object.
...
The hourly flight density estimation is based on flown \acr{IFR} trajectories as
...
H3 cells (red) at resolution 3 and their centres (blue) covering the bounding
...
<!-- next sentence feels like an orphan ... do we need this here? ------>
Next sections will develop the mathematical background for the evaluation of collision expectation of which traffic density is a key component.
...
(Ukraine, Libya or Syria). The major traffic flows (North Atlantic, Canaries
...
Uber's H3 method ensures non-overlapping hexagons (and 5 pentagons), with an equidistant relationship between all neighbours, and enables an efficient indexing of the tessel-cells. 
...
<!-- TERMINOLOGY? rectangular solid -- impact top plane, front plane? Does the literature use "view"  -->
...
At the same time, the falling of the re-entering debris object at regular en-route flight levels will be vertical[^ballistic_descent].
...
* constant aircraft speed, $s_{a} = C_t$, equal to its cruise speed, under the
  simplification that the majority of the flight time spent cruising, 
...
The values $W_t$, $H_t$, $L_t$ and $C_{t}$ have been retrieved from [@performance_database;@skybrary;@doc8643]; we used $\varepsilon_t = \qty{1000}{m^2}$ missing entries.
...
The angular position, $\theta = \omega + \nu$, of the re-entry point from the
ascending node, see @sec-orbitals, is uniformly distributed for uncontrolled
re-entries, $\sigma(\theta) = 1 / (2\pi)$.
This is due to uncertainties in atmospheric drag (among other things solar
activity can expand the atmosphere) and the gravitational effects of the
Equatorial bulge.
Hence we assume that the impact longitude, $\lambda$, is uniformly distributed.
...
This density has been derived by simulating a full circular orbit at
\qty{550}{km} height with 10000  positions and calculating the relevant longitudes and latitudes at half degree resolution.
...
If we consider historical uncontrolled debris re-entry data, we can numerically
...
\acr{NM} area we obtain the weightings per cell (per hour $H$), $W_H(h)$, as in
...
In few days (at the time of writing of this paper) at 23:06 May 10, 2025
($\pm 1.8$ days), the KOSMOS 482 Descent Craft should re-enter the
Earth's atmosphere [@kosmos482reentry].
Supposedly the lander module from a 1972 failed USSR Venera mission [@SpaceReviewKosmos],
...
The densest cell, `83194afffffffff` (area \qty{11131585027}{m^2}), at 24.56 flights/hour has centroid latitude
of \ang{50.94369} so within the latitude range of $[\ang{-51.7}, \ang{51.7}]$.
Using an average value for effective exposed area of \qty{1000}{m^2},
we can quickly calculate the collision expectation as

$$
\begin{aligned}
E &= w_h \cdot \sigma_h \cdot \bar{\varepsilon} / area_h \\
  &= 10^{-6} \cdot 24.56 \cdot 1000 / 11131585027 = 2.2\cdot 10^{-12}
\end{aligned}
$$
...
We have calculated collision and casualty risk probabilities for the two
...
GitHub repository of the paper at <https://github.com/euctrl-pru/flight_density_space_debris>.
We have also developed a companion R package `aviodebris` to heavy-lift the
various data processing steps to produce the results in the paper;
it is available at <https://github.com/euctrl-pru/aviodebris>.
