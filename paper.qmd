...
The probability of impact for debris object $k$, $p_k$, is expressed as.

$$
p_k = \varepsilon_k \int_a \rho(\vec{x}) \sigma(\vec{x}) da
$$ {#eq-impact-probability}

where $\rho({\vec{x}})$ is the probability density of an aircraft
and $\sigma(\vec{x})$ is probability density of a debris object in the
position $\vec{x}$.
...
circular orbits prior to reentry even if initially on an elliptical ones.
...
So for example [@patera2008], if a building is located at latitude 30 degrees
...
The effective expose area, $\varepsilon_i$, is
$$
